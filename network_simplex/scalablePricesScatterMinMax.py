        ...
        if timestamp not in timestamp_dict.keys():
            ...
            if isinstance(tradingPairs, dict):
                for matchedhouseholds in tradingPairs.keys():
                    if matchedhouseholds[0] != 'supplyDummy' and matchedhouseholds[1] != 'demandDummy':
...
def plot_runtime_scatter(timestamps,timeForTicker, sorted_trading_prices, sorted_max_reachedPrice, sorted_min_reachedPrice,
    ...
    # create title
    averageCommunityPrice.set_title('Netzwerk-Simplex: Preisverteilung 01. Juli 2020')
   ...
   # from matplotlib.lines import Line2D
    #legend_elements = [Line2D([0], [0], color='black', label='höchster Strompreis'),
     #                  Line2D([0], [0], color='green', label='niedrigster Strompreis'),
      #                 Line2D([0], [0], color='red', label='durchschnittlicher Strompreis'),
       #                Line2D([0], [0], marker='o', color='w', label='erreichte Strompreise',
...
#    plt.legend(handles=legend_elements, loc='best')
   ...
   # fig.savefig("legende_netz_scatter_min_max.svg")
    fig.savefig("alt0107netz_scatter_min_max.pdf")
