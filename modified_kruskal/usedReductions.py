...
def plot_usedReductions(timestamps,timeForTicker,nested_netCostDifference, nested_numberOfPairs, nested_lokalDistance, nested_konzessionsDifference):
    ...
    # create title
    averageCommunityPrice.set_title('mod. Kruskal: eingesetzte rechtliche Rahmenbedingungen 01. Juli 2020')

    # set axes labels
    averageCommunityPrice.set_xlabel("Uhrzeit")
    averageCommunityPrice.set_ylabel("rechtliche Rahmenbedingungen")
    averageCommunityPrice.set_ylim(0,900)
    ...
    legend_elements = [Line2D([0], [0], color='green', label='Anzahl an Haushaltepaaren'),
                       Line2D([0], [0], color='black', label='eingesetzter lokaler Zusammenhang'),
                       Line2D([0], [0], color='red', label='eingesetzte Konzessionsabgabendifferenzen'),
                       Line2D([0], [0], color='blue', label='eingesetzte Netzentgeltdifferenzen')]
#
    ...
    fig.savefig("legende_kruskal_usedReductions.svg")
    #fig.savefig("kruskal_usedReductions.pdf")
...
def main():
    ...
    data = get_day_data('01_07_nurReduzierung.csv')
